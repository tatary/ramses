...
subroutine build_comm(ilevel)
  ...
  integer,dimension(ncpu)::reqsend,reqrecv
#endif
#ifdef MAX_COMM_RANKS
  integer,parameter::N_limit=MAX_COMM_RANKS
  integer::phase,num_phases,num_groups
  integer::sender_phase,receiver_phase
  integer::sender_start,sender_end
  integer::receiver_start,receiver_end
  logical::is_sender,is_receiver
  integer::rank
  integer,dimension(MPI_STATUS_SIZE,ncpu)::send_statuses,recv_statuses
  integer,dimension(N_limit)::lc_reqrecv
#endif
  ...
  ! Wait for full completion of sends and receives
  call MPI_WAITALL(countsend,reqsend,statuses,info)
  call MPI_WAITALL(emission(ilevel)%nactive,reqrecv,statuses,info)
#else
  ! Pre-populate cpuid and ngrids before phase loop
  idx=1
  do icpu=1,ncpu
    if(icpu==myid) cycle
    ngrids=recvbuf(icpu)
  ...
  ! Split communication: divide ranks into groups of N_limit
  num_groups=ceiling(real(ncpu)/real(N_limit))
  num_phases=num_groups*num_groups
  do phase=1,num_phases
    sender_phase=(phase-1)/num_groups
    receiver_phase=mod(phase-1,num_groups)
    sender_start=sender_phase*N_limit
    sender_end=MIN(sender_start+N_limit-1,ncpu-1)
    receiver_start=receiver_phase*N_limit
    receiver_end=MIN(receiver_start+N_limit-1,ncpu-1)
    is_sender=(rank>=sender_start .and. rank<=sender_end)
    is_receiver=(rank>=receiver_start .and. rank<=receiver_end)
    reqsend=MPI_REQUEST_NULL
    countsend=0
    if(is_sender)then
      do icpu=receiver_start+1,receiver_end+1
        ncache=reception(icpu,ilevel)%ngrid
        if(ncache>0)then
          countsend=countsend+1
          call MPI_ISEND(reception(icpu,ilevel)%pcomm%f,ncache, &
          & MPI_INTEGER,icpu-1,tag+phase,MPI_COMM_WORLD,reqsend(countsend),info)
        end if
      end do
    end if
    countrecv=0
    if(is_receiver)then
      offset=1
      do idx=1,emission(ilevel)%nactive
        icpu=emission(ilevel)%cpuid(idx)
        ngrids=emission(ilevel)%ngrids(idx)
        if(icpu-1>=sender_start .and. icpu-1<=sender_end)then
          countrecv=countrecv+1
          call MPI_IRECV(emission(ilevel)%igrid(offset),ngrids, &
          & MPI_INTEGER,icpu-1,tag+phase,MPI_COMM_WORLD,lc_reqrecv(countrecv),info)
        end if
        offset=offset+ngrids
      end do
    end if
    call MPI_WAITALL(countsend,reqsend,send_statuses,info)
    call MPI_WAITALL(countrecv,lc_reqrecv,recv_statuses,info)
  ...
  do phase=1,num_phases
    ...
    countrecv=0
    if(is_receiver)then
      do icpu=sender_start+1,sender_end+1
        ncache=emission(icpu,ilevel)%ngrid
        if(ncache>0)then
          countrecv=countrecv+1
          call MPI_IRECV(emission(icpu,ilevel)%igrid,ncache, &
          & MPI_INTEGER,icpu-1,tag+phase,MPI_COMM_WORLD,reqrecv(countrecv),info)
        end if
      end do
    end if
    call MPI_WAITALL(countsend,reqsend,send_statuses,info)
    call MPI_WAITALL(countrecv,reqrecv,recv_statuses,info)
    call MPI_Barrier(MPI_COMM_WORLD,info)
  end do
#endif
#endif
