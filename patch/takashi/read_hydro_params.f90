subroutine read_hydro_params(nml_ok)
  ...
  & ,strict_equilibrium &
  & ,pressure_fix_cond, pressure_fix_maxMach
...
110 rewind(1)
  ...
  ! Check pressure-fix parameters
  if (pressure_fix_cond < 0 .or. pressure_fix_cond > 2) then
    if (myid == 1) write(*,*) 'Error in the namelist'
    if (myid == 1) write(*,*) 'pressure_fix_cond must be 0, 1, or 2'
    nml_ok = .false.
  end if

  if (pressure_fix_cond == 2 .and. pressure_fix_maxMach <= 0d0) then
    if (myid == 1) write(*,*) 'Error in the namelist'
    if (myid == 1) write(*,*) &
      'pressure_fix_maxMach must be positive when pressure_fix_cond = 2'
    nml_ok = .false.
  end if
  rewind(1)
