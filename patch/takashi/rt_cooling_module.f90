...
module rt_cooling_module
  ...
  ! (TO ADDED) relax_x_by_ne relaxes the per-species subcycle constraints
  ! (code=6 hydrogen, code=7 helium) by also accepting the step when the
  ! ELECTRON budget barely moves. See the hydrogen block in cool_step for the
  ! full reasoning. It is a &RT_PARAMS namelist variable declared in
  ! rt_parameters.f90 (default .false. = previous behaviour), so switching it
  ! needs no rebuild.
  ...
  real(kind=8), allocatable, dimension(:) :: log_T_cloudy
  real(kind=8), allocatable, dimension(:) :: log_nH_cloudy
  real(kind=8), allocatable, dimension(:) :: zred_cloudy
  real(kind=8), allocatable, dimension(:, :, :) :: met_cool_tab
  real(kind=8), allocatable, dimension(:, :, :) :: met_heat_tab
  real(kind=8), allocatable, dimension(:, :, :) :: met_cool_prime_tab
  real(kind=8), allocatable, dimension(:, :, :) :: met_heat_prime_tab
...
!-------------------------------------------------------------------------
    ...
    if(cosmo) then
      ...
      if (cloudy_metal_cooling) then
        call init_cloudy_metal_cooling
      end if
    else
...
!-------------------------------------------------------------------------
  ...
  contains
    ...
    SUBROUTINE cool_step(icell)
      ...
      !
      ! The timestep constraints below are written as .not.(dUU .le. 1) rather
      ! than (dUU .gt. 1), so that a NaN in dUU rejects the substep (and halves
      ! ddt) instead of silently passing the test: every comparison with a NaN
      ! is false, so the (dUU .gt. 1) form accepts NaN as a valid solution.
      !
      ...
      real(dp),save:: f_ne_H, f_ne_He   ! |dx| -> |dne|/ne conversion (TO ADDED)
      ...
      real(dp), save:: metal_cool_tot, metal_cool_prime, metal_heat_tot &
        , metal_heat_prime
      ...
      if(self_shielding .and. .not. cloudy_metal_cooling) then
        ss_factor = exp(-nH(icell)/1d-2)
      else if (self_shielding .and. cloudy_metal_cooling) then
        ss_factor = ss_factor_rahmati(nH(icell), 1.d0/aexp - 1.d0)
      end if
      ...
      if(c_switch(icell) .and. cooling .and. .not. rt_T_rad) then
        ...
        if (cloudy_metal_cooling) then
          metal_cool_tot = 0d0
          metal_cool_prime = 0d0
          metal_heat_tot = 0d0
          metal_heat_prime = 0d0
        endif
        if (Zsolar(icell) .gt. 0d0) then
          if (cloudy_metal_cooling) then
            call rt_cmp_cloudy_metals(T2(icell), nH(icell), mu &
              , metal_cool_tot, metal_cool_prime           &
              , metal_heat_tot, metal_heat_prime, a_exp)
          else
        ...
        if (cloudy_metal_cooling) then
          rate = X_nHkb*(Hrate - Crate - &
            Zsol_cloudy_factor*Zsolar(icell)*(metal_cool_tot - metal_heat_tot))
          dRate = -X_nHkb*(dCdT2 + &
            Zsol_cloudy_factor*Zsolar(icell)*(metal_cool_prime - metal_heat_prime)) ! dRate/dT2
        else
      ...
      if(rt_isIR) then
        if(kAbs_loc(iIR) .gt. 0d0 .and. .not. rt_T_rad) then
          ...
          dUU = ABS(dNp(iIR)-Np(iIR,icell)) / (Np(iIR,icell)+Np_MIN)     &
            * one_over_Np_FRAC
          if(.not. (dUU .le. 1.)) then
            code=4 ;   RETURN
          endif
          fracMax=MAX(fracMax,dUU)

          dUU   = ABS(dT2-T2(icell)) / (T2(icell)+T_MIN) * one_over_T_FRAC
          if(.not. (dUU .le. 1.)) then                           ! 10% rule for T2
            code=5 ; RETURN
          endif
          fracMax=MAX(fracMax,dUU)
      ...
      !--------------------------------------------------------------- TO ADDED
      ! The bare |dx_i|/(x_i + x_FM) tests degrade in highly ionised gas: the
      ! denominator is the species abundance itself, so a physically irrelevant
      ! absolute change in a species that has almost vanished counts as a large
      ! relative change. The x_FM = 1d-6 floor already caps the worst of this
      ! (a companion code with a 1d-12 floor stalled completely on exactly this,
      ! with x_HI ~ 1d-8 cells taking >13000 subcycles), but the 1d-6 to 1d-3
      ! band is still purely relative, and that is the band an ionisation front
      ! sweeps through.
      !
      ! Accept the step if EITHER relative change is small:
      !   |dx_i| / x_i        - meaningful while species i is abundant
      !   |dne|  / ne         - meaningful once it is not
      ! One of the two is always well conditioned, so they cannot both be
      ! pathological. MIN (not MAX) is what loosens: dUU large = reject.
      ! The outer MAX over species is kept, so no species loses its guard.
      !
      ! dne is bounded from the atomic conservation of each element:
      !   H : 2*dxH2 + dxHI + dxHII = 0, so |dne| = nH*|dx| for HI/HII -> use nH
      !   xH2 is deliberately LEFT ALONE: H2 carries no electrons, so tying its
      !   tolerance to the electron budget would only be an indirect argument
      !   via conservation. Its own relative test is kept unchanged.
      !   He: dxHeI + dxHeII + dxHeIII = 0 and HeIII carries two electrons,
      !       so |dne| <= 2*nHe*|dx| for every He species -> use 2*nHe
      ! The larger factor is the conservative (stricter) choice.
      ! ne itself is guarded independently by code=8 further down, so relaxing
      ! these tests does not lose control of the electron density.
      !
      ! Set relax_x_by_ne = .false. to restore the old behaviour.
      dUU = 0d0
      if(isH2) dUU = MAX(dUU,ABS((dxH2-xH2)/(xH2+x_FM)))   ! xH2 は従来のまま
      if (relax_x_by_ne) then
        f_ne_H = nH(icell)/(neInit+x_FM*nH(icell))
        dUU = MAX(dUU, MIN(ABS(dxHI-xHI)/(xHI+x_FM)                        &
        &               ,ABS(dxHI-xHI)*f_ne_H))
        dUU = MAX(dUU, MIN(ABS(dXion(ixHII)-xion(ixHII,icell))             &
        &                 /(xion(ixHII,icell)+x_FM)                      &
        &               ,ABS(dXion(ixHII)-xion(ixHII,icell))*f_ne_H))    &
          * one_over_x_FRAC
      else
      ...
      if(isHe) then
        ...
        ! (TO ADDED) same MIN-with-the-electron-budget treatment as the hydrogen
        ! block above; see the comment there. 2*nHe because HeIII carries two
        ! electrons and He conservation can route any change into it.
        if (relax_x_by_ne) then
          f_ne_He = 2d0*nHe/(neInit+x_FM*nH(icell))
          dUU = MIN(ABS(dxHeI-xHeI)/(xHeI+x_FM), ABS(dxHeI-xHeI)*f_ne_He)
          dUU = MAX(dUU, MIN(ABS(dxion(ixHeII)-xion(ixHeII,icell))          &
          &                  /(xion(ixHeII,icell)+x_FM)                   &
          &               ,ABS(dxion(ixHeII)-xion(ixHeII,icell))*f_ne_He))
          dUU = MAX(dUU, MIN(ABS(dxion(ixHeIII)-xion(ixHeIII,icell))        &
          &                  /(xion(ixHeIII,icell)+x_FM)                  &
          &               ,ABS(dxion(ixHeIII)-xion(ixHeIII,icell))*f_ne_He)) &
            * one_over_x_FRAC
        else
      ...
      ! ne is a number density [cm-3], so the x_FM floor (an ionisation
      ! fraction) must be scaled by nH to be dimensionally consistent:
      dUU=ABS((ne-neInit)) / (neInit+x_FM*nH(icell)) * one_over_x_FRAC
      if(.not. (dUU .le. 1.)) then
        code=8 ; RETURN
      endif
      fracMax=MAX(fracMax,dUU)
...
!************************************************************************
!=======================================================================
  SUBROUTINE init_cloudy_metal_cooling
!=======================================================================
    ! This subroutine is written by Takashi Okamoto (t.t.okamoto@gmail)
    ! It reads in the metal contributions to the coolgin and heating rates
    ! calculated by CLOUDY (Ferland et al. 2013) and stores them in the
    ! arrays.
    ! Note that the UV background is available for z < 10.0 only.
    !-------------------------------------------------------------
    use amr_commons, only: myid
#ifndef WITHOUTMPI
    ...
    if (.not. ok) then
      write (*, *) 'WARNING: CLOUDY metal cooling file '// &
      & TRIM(cloudy_metal_file)//' not found.'
      call clean_stop
    end if
    ...
    open (unit=880, file=TRIM(cloudy_metal_file), form='unformatted')
    ! read the number of elements in each array
    read (880) nbin_T_cloudy
    read (880) nbin_n_cloudy
    read (880) nbin_z_cloudy

    allocate (table_to_read(1:nbin_T_cloudy))
    allocate (log_T_cloudy(1:nbin_T_cloudy))
    allocate (log_nH_cloudy(1:nbin_n_cloudy))
    allocate (zred_cloudy(1:nbin_z_cloudy))

    allocate (met_cool_tab(1:nbin_T_cloudy, 1:nbin_n_cloudy, 1:nbin_z_cloudy))
    allocate (met_heat_tab(1:nbin_T_cloudy, 1:nbin_n_cloudy, 1:nbin_z_cloudy))
    allocate (met_cool_prime_tab(1:nbin_T_cloudy, 1:nbin_n_cloudy, 1:nbin_z_cloudy))
    allocate (met_heat_prime_tab(1:nbin_T_cloudy, 1:nbin_n_cloudy, 1:nbin_z_cloudy))

    read (880) log_T_cloudy
    read (880) log_nH_cloudy
    read (880) zred_cloudy

    ! read the cooling and heating rates
    do j = 1, nbin_z_cloudy
      do i = 1, nbin_n_cloudy
        read (880) table_to_read
        met_cool_tab(:, i, j) = table_to_read
        read (880) table_to_read
        met_cool_prime_tab(:, i, j) = table_to_read
        read (880) table_to_read
        met_heat_tab(:, i, j) = table_to_read
        read (880) table_to_read
        met_heat_prime_tab(:, i, j) = table_to_read
      end do
    end do
    ...
    dlogT = log_T_cloudy(2) - log_T_cloudy(1)
    dlogN = log_nH_cloudy(2) - log_nH_cloudy(1)
    deallocate (table_to_read)
    ! Send the token
#ifndef WITHOUTMPI
    if (IOGROUPSIZE > 0) then
  ...
  end subroutine init_cloudy_metal_cooling

!************************************************************************
  !=========================================================================
  function bilinear_interpolation(inH, iT, lnH, Temp, table) &
    result(value)
    ! This subroutine is written by Takashi Okamoto (t.t.okamoto@gmail)
    ! This applies bi-linear interpolation to the cooling/heating tables
    !-------------------------------------------------------------------------
    ! x: Temp, y: zred
    implicit none
    integer::inH, iT
    real(dp)::lnH, Temp
    real(kind=8), dimension(1:nbin_T_cloudy, 1:nbin_n_cloudy, 1:nbin_z_cloudy) &
      ::table
    real(dp)::T0, T1, dT
    real(dp)::xd, yd
    real(dp)::c00, c10, c01, c11
    real(dp)::c0, c1
    real(dp)::value

    T0 = 10**log_T_cloudy(iT)
    T1 = 10**log_T_cloudy(iT + 1)
    xd = (Temp - T0)/(T1 - T0)
    yd = (lnH - log_nH_cloudy(inH))/(log_nH_cloudy(inH + 1) - log_nH_cloudy(inH))

    c00 = table(iT, inH, nbin_z_cloudy)
    c01 = table(iT, inH + 1, nbin_z_cloudy)
    c10 = table(iT + 1, inH, nbin_z_cloudy)
    c11 = table(iT + 1, inH + 1, nbin_z_cloudy)

    c0 = c00*(1d0 - xd) + c10*xd
    c1 = c01*(1d0 - xd) + c11*xd

    value = c0*(1d0 - yd) + c1*yd
  end function bilinear_interpolation

  !=========================================================================
  function trilinear_interpolation(iZ, inH, iT, zred, lnH, Temp, table) &
    result(value)
    ! This subroutine is written by Takashi Okamoto (t.t.okamoto@gmail)
    ! This applies tri-linear interpolation to the cooling/heating tables
    !-------------------------------------------------------------------------
    ! x: Temp, y: lnH, z: zred
    implicit none
    integer::iZ, inH, iT
    real(dp)::zred, lnH, Temp
    real(kind=8), dimension(1:nbin_T_cloudy, 1:nbin_n_cloudy, 1:nbin_z_cloudy) &
      ::table
    real(dp)::T0, T1, dT
    real(dp)::xd, yd, zd
    real(dp)::c000, c100, c001, c101, c010, c110, c011, c111
    ...
    xd = (Temp - T0)/(T1 - T0)
    yd = (lnH - log_nH_cloudy(inH))/(log_nH_cloudy(inH + 1) - log_nH_cloudy(inH))
    zd = (zred - zred_cloudy(iZ))/(zred_cloudy(iZ + 1) - zred_cloudy(iZ))

    c000 = table(iT, inH, iZ)
    c100 = table(iT + 1, inH, iZ)
    c001 = table(iT, inH, iZ + 1)
    c101 = table(iT + 1, inH, iZ + 1)
    c010 = table(iT, inH + 1, iZ)
    c110 = table(iT + 1, inH + 1, iZ)
    c011 = table(iT, inH + 1, iZ + 1)
    c111 = table(iT + 1, inH + 1, iZ + 1)

    c00 = c000*(1d0 - xd) + c100*xd
    c01 = c001*(1d0 - xd) + c101*xd
    c10 = c010*(1d0 - xd) + c110*xd
    c11 = c011*(1d0 - xd) + c111*xd
    c0 = c00*(1d0 - yd) + c10*yd
    c1 = c01*(1d0 - yd) + c11*yd
    value = c0*(1d0 - zd) + c1*zd
  end function trilinear_interpolation
  !=========================================================================
  ! This function is written by Takashi Okamoto (t.t.okamoto@gmail)
  ! It returns the self-shielding factor for the UV background
  ! for given set of parameters
  function rahmati_fitting_func(nH, nH0, f, alpha1, alpha2, beta) &
    result(value)
    real(dp)::nH, nH0, f, alpha1, alpha2, beta
    real(dp)::value
    value = (1.0 - f)*((1.0 + (nH/nH0)**beta)**alpha1) &
      + f*(1.0 + nH/nH0)** alpha2
  end function rahmati_fitting_func
  !=========================================================================
  ! This function is written by Takashi Okamoto (t.t.okamoto@gmail)
  ! This applies linear interpolation in log space
  !-------------------------------------------------------------------------
  function linear_interpolation_in_log(x, a, b, fa, fb) result(value)
    implicit none
    real(dp)::x, a, b, fa, fb
    real(dp)::value
    real(dp)::logfa, logfb, grad
    value = 10.d0**(log10(fa) + (log10(fb) - log10(fa))/(b - a)*(x - a))
  end function linear_interpolation_in_log
  !=========================================================================
  ! This function is written by Takashi Okamoto (t.t.okamoto@gmail)
  ! It returns self-shielding factor for the UV background
  ! The function is based on Rahmati et al. 2013
  function ss_factor_rahmati(nH, zred) result(value)
    integer::i
    real(dp)::nH, zred
    real(dp)::value
    real(dp)::nH0, f, alpha1, alpha2, beta
    ! Fitting parameters by Chardin et al. 2018
    integer, parameter::n_bin_rahmati=8
    real(dp), parameter::rahmati_redshift(n_bin_rahmati)=(/ &
    & 3.0d0, 4.0d0, 5.0d0, 6.0d0, 7.0d0, 8.0d0, 9.0d0, 10.0d0 /)
    real(dp), parameter::rahmati_n0(n_bin_rahmati)=(/ &
    & 0.0090, 0.0093, 0.0103, 0.0070, 0.0027, 0.0040, 0.0046, 0.0047/)
    real(dp), parameter::rahmati_alpha1(n_bin_rahmati)=(/ &
    & -1.12, -0.95, -1.29, -0.94, -0.86, -0.74, -0.64, -0.39 /)
    real(dp), parameter::rahmati_alpha2(n_bin_rahmati)=(/ &
    & -1.65, -1.50, -1.60, -1.51, -1.27, -1.40, -1.21, -0.86 /)
    real(dp), parameter::rahmati_beta(n_bin_rahmati)=(/ &
    & 5.32, 5.87, 5.06, 6.11, 7.08, 7.12, 9.99, 12.94 /)
    real(dp), parameter::rahmati_f(n_bin_rahmati)=(/ &
    & 0.018, 0.015, 0.024, 0.029, 0.041, 0.041, 0.029, 0.006 /)

    if (zred <= rahmati_redshift(1)) then
      value = rahmati_fitting_func(nH, rahmati_n0(1), rahmati_f(1), &
        rahmati_alpha1(1), rahmati_alpha2(1), &
        rahmati_beta(1))
    else if (zred >= rahmati_redshift(n_bin_rahmati)) then
      value = rahmati_fitting_func(nH, rahmati_n0(n_bin_rahmati), &
        rahmati_f(n_bin_rahmati), &
        rahmati_alpha1(n_bin_rahmati), &
        rahmati_alpha2(n_bin_rahmati), &
        rahmati_beta(n_bin_rahmati))
    else
      if (zred < rahmati_redshift(2)) then
        i = 1
      else if (zred < rahmati_redshift(3)) then
        i = 2
      else if (zred < rahmati_redshift(4)) then
        i = 3
      else if (zred < rahmati_redshift(5)) then
        i = 4
      else if (zred < rahmati_redshift(6)) then
        i = 5
      else if (zred < rahmati_redshift(7)) then
        i = 6
      else
        i = 7
      end if
      nH0 = linear_interpolation_in_log(zred, rahmati_redshift(i), &
        rahmati_redshift(i + 1), &
        rahmati_n0(i), rahmati_n0(i + 1))
      f = linear_interpolation_in_log(zred, rahmati_redshift(i), &
        rahmati_redshift(i + 1), &
        rahmati_f(i), rahmati_f(i + 1))
      alpha1 = linear_interpolation_in_log(zred, rahmati_redshift(i), &
        rahmati_redshift(i + 1), &
        rahmati_alpha1(i), rahmati_alpha1(i + 1))
      alpha2 = linear_interpolation_in_log(zred, rahmati_redshift(i), &
        rahmati_redshift(i + 1), &
        rahmati_alpha2(i), rahmati_alpha2(i + 1))
      beta = linear_interpolation_in_log(zred, rahmati_redshift(i), &
        rahmati_redshift(i + 1), &
        rahmati_beta(i), rahmati_beta(i + 1))
      value = rahmati_fitting_func(nH, nH0, f, alpha1, alpha2, beta)
    end if

  end function ss_factor_rahmati

  !=========================================================================
  subroutine rt_cmp_cloudy_metals(T2, nH, mu, metal_cool_tot, metal_cool_prime &
    , metal_heat_tot, metal_heat_prime, aexp)
    ! This subroutine is written by Takashi Okamoto (t.t.okamoto@gmail)
    ! It computes the contribution of metals to the cooling and heating rates
    ! T2                 => Temperature in Kelvin, divided by mu
    ! nH                 => Hydrogen number density (H/cc)
    ! mu                 => Average mass per particle in terms of mH
    ! metal_cool_tot     <= Metal cooling contribution to de/dt [erg s-1 cm-3]
    ! metal_cool_prime   <=  d(metal_tot)/dT2 [erg s-1 cm-3 K-1]
    ! metal_heae_tot     <= Metal heating contribution to de/dt [erg s-1 cm-3]
    ! metal_heat_prime   <=  d(metal_heat)/dT2 [erg s-1 cm-3 K-1]
    !=========================================================================
    implicit none
    real(dp) ::T2, nH, mu, metal_cool_tot, metal_cool_prime
    real(dp) :: metal_heat_tot, metal_heat_prime, aexp
    real(dp) :: log_1pz, TT, lTT, lnH
    real(dp) :: var0, var1, T0, T1
    real(dp) :: zred
    integer ::iTemp, izred, inH

    zred = max(0.0d0, 1.0d0/aexp - 1.0d0)
    TT = T2*mu
    lTT = log10(TT)
    lnH = log10(nH)
    if (zred .ge. z_reion) then
      ...
      log_1pz = log10(zred + 1.0d0)
      izred = (log_1pz - log_1pzmin)/(dlogz) + 1
    end if

    if (zred .ge. z_reion) then
      if (lTT .ge. log_T_cloudy(nbin_T_cloudy)) then
        metal_cool_tot = 0d0
        metal_cool_prime = 0d0
        metal_heat_tot = 0d0
        metal_heat_prime = 0d0
        ...
        T0 = 10d0**log_T_cloudy(iTemp)
        T1 = 10d0**log_T_cloudy(iTemp + 1)
        if ((lnH .lt. log_nH_cloudy(1)) .or. &
          lnH .ge. log_nH_cloudy(nbin_n_cloudy)) then
          if (lnH .lt. log_nH_cloudy(1)) then
            inH = 1
          else
            inH = nbin_n_cloudy
          end if
          ...
          metal_cool_tot = bilinear_interpolation(inH, iTemp, lnH, TT, met_cool_tab)
          metal_cool_prime = bilinear_interpolation(inH, iTemp, lnH, TT, &
            met_cool_prime_tab)
          metal_heat_tot = bilinear_interpolation(inH, iTemp, lnH, TT, met_heat_tab)
          metal_heat_prime = bilinear_interpolation(inH, iTemp, lnH, TT, &
            met_heat_prime_tab)
        end if
      else
        metal_cool_tot = 0d0
    ...
    else ! table with a UV-background
      if (lTT .ge. log_T_cloudy(nbin_T_cloudy)) then
        metal_cool_tot = 0d0
        metal_cool_prime = 0d0
        metal_heat_tot = 0d0
        ...
        metal_cool_prime = trilinear_interpolation(izred, inH, iTemp, zred, lnH, &
          TT, met_cool_prime_tab)
        metal_heat_tot = trilinear_interpolation(izred, inH, iTemp, zred, lnH, &
          TT, met_heat_tab)
        metal_heat_prime = trilinear_interpolation(izred, inH, iTemp, zred, lnH, &
          TT, met_heat_prime_tab)
      else
        metal_cool_tot = 0d0
        metal_cool_prime = 0d0
        metal_heat_tot = 0d0
    ...
    metal_cool_tot = metal_cool_tot*nH**2
    metal_cool_prime = metal_cool_prime*nH**2*mu
    metal_heat_tot = metal_heat_tot*nH
    metal_heat_prime = metal_heat_prime*nH*mu
  end subroutine rt_cmp_cloudy_metals
