  ...
  do ind=1,twotondim
    ...
    if(pressure_fix)then
      ...
      do i=1,active(ilevel)%ngrid
        ...
        select case(pressure_fix_cond)
         case(0)
          ! Original RAMSES criterion
          if (e_cons < e_trunc) then
            uold(ind_cell,ndim+2) = e_prim + e_kin
          end if

         case(1)
          ! Apply only when the primitive update increases thermal energy
          if (e_cons < e_trunc .and. e_cons < e_prim) then
            uold(ind_cell,ndim+2) = e_prim + e_kin
          end if

         case(2)
          ! Apply only in high-Mach flows
          if (e_cons < e_trunc .and. &
            e_cons < 2d0 * e_kin / pressure_fix_maxMach**2) then
            uold(ind_cell,ndim+2) = e_prim + e_kin
          end if
        end select
      end do
