!=============================================================================
!                   SUPER MEGA GIGA COOLING MODULE
!=============================================================================
! Subroutines and functions of general interest are :
!
! ROUTINES TO BE CALLED BY HYDRO CODE
!
!    subroutine set_model(...) :
!          to select the cooling model and its parameters
!
!    subroutine set_table(aexp) :
!          to create table with default parameters
!          More convenient to call than
!          cmp_table(nH_min,nH_max,T2_min,T2_max,nbin_n,nbin_T,aexp)
!
!    subroutine solve_cooling(...) :
!          to calculate cooling
!
! ROUTINE TO BE MODIFIED IF NECESSARY
!
!    function J0simple(aexp) :
!          gives J0 as a function of redshift in the Teyssier
!          or Theuns
!
!=============================================================================
module cooling_module
  ...
  ! Use table%n_spec if necessary
  logical, parameter :: if_species_abundances=.true.
  ! Corrective factor of Theuns et al.
  real(kind=8),parameter :: dumfac_ion_theuns=2d0
  real(kind=8),parameter :: dumfac_rec_theuns=0.75D0  ! the same
  real(kind=8) :: dumfac_ion=dumfac_ion_theuns
  real(kind=8) :: dumfac_rec=dumfac_rec_theuns

  ! WE MUST HAVE OR teyssier OR theuns OR madau
  ! OR weinbergint OR courty with an exclusive OR
  ...
  ! If teyssier or theuns :
  real(kind=8) :: J0in=1d-22  ! J0 default
  real(kind=8) :: J0min=1d-29 ! Minimum J0 value
  logical :: force_j0_one=.false. ! Force constant UV bkg
  ! (saturation at high redshift)
  real(kind=8) :: aexp_ref=0.0001d0
  real(kind=8) :: J0min_ref=2.77168510365299962D-25 ! J0min_ref precalculates
  ! for H0=70, omegab=0.04, omega0=0.3, omegaL=0.7
  logical :: high_z_realistic_ne=.true. ! Calculate J0min so that n_e is
  ! realistic at large z. J0min=J0min_ref/(aexp/aexp_ref)^2
  real(kind=8) :: alpha=1d0   ! J(nu) \propto nu^{-alpha}
  ! If madau ou weinbergint :
  real(kind=8) :: normfacJ0=0.74627d0   ! Normalization factor
  ! for J0 for a J(nu,z) of the Haardt and Madau type
  ! This is the factor used by Dave et al. for LCDM
  ! Saving of cooling/heating terms in
  ...
  ! The heating and photoionization rates of Dave et al.
  ! for J0 latest version of HM (weinberg or weinbergint if
  ! if_read_weinberg=.true. (see below) in the latter case)
  real(kind=8),allocatable, dimension(:,:)::table_weinberg
  ! Interpolation table in input
  character(len=128), parameter :: table_weinberg_name='TREECOOL'
  ! File name with data
  integer,parameter :: luweinberg=21
  ! unit to read the file
  integer :: Nweinberg
  ! Number of redshift bins

  ! Heating rate interpolation coefficients from Dave et al.
  ! (weinbergint)
  logical,parameter :: if_read_weinberg=.false.
  ! .true. to read the table_weinberg_name file
  ! then interpolate by a polynomial
  ! .false. to use coefficient values
  ! precalculated listed below

  integer,parameter :: Norderweinberg=7
  ! Order+1 of interpolation polynomial (DO NOT CHANGE)
...
!=======================================================================
! Nmodel(integer) =1 : Teyssier : former choice of evolution and shape of J(nu,z)
!                  2 : Theuns   : same but with Theuns interpolated functions (faster)
!                  3 : Madau    : J(nu,z) of Theuns et al. 1998 with the old measurements of
!                                 Haardt et Madau (HM)
!                  4 : Weinberg : J(nu,z) from Dave et al. 1999 with new HM measurements
!                                 read from table_weinberg_name file (inactive)
!                  5 : the same as 4 but interpolates polynomially: RECOMMENDED
!                  6 : Courty
!                 -1 : default defined in the module
! J0in_in (dble): J0 value used for Teyssier and Theuns
!            Exemple : J0in_in=1d-22
!            J0in_in <= 0 uses the default defined in the module
! J0min_in (dble) :J0min or J0min_ref value (see realistic_ne option)
!            used in all high redshift models
!            Exemple : J0min_in=1d-29
!            J0min_in <= 0 uses the default defined in the module
! alpha_in (dble) : value of the spectral index of J(nu) \propto nu^{-alpha}
!            Exemple : alpha=1.
!            alpha_in < 0 uses the default defined in the module
! zreioniz_in (dble) : reionization redshift value
!            Exemple : zerion=10.
!            zreioniz_in < 0 uses the default defined in the module
! normfacJ0_in (dble) : value of the normalization factor in the case of
!            Haardt and Madau spectra. It's a number of the order of
!            unity, generally smaller than 1.
!            Exemple : normfacJ0_in=0.74627
!            normfacJ0_in takes the default defined in the module
! correct_cooling (integer) : 0 : no correction
!                             1 : correction of Theuns et al 98
!                            -1 : default defined in the module
! realistic_ne (integer) : 0 : no realistic n_e at big redshift :
!                               J0min remains the same regardless of redshifti
!                              (J0min=J0min_in if redshift > 0)
!                          1 : n_e realistic at high redshift: J0min proportional to 1/a^2
!                              initially equal to J0min_ref for a=aexp_ref=0.0001
!                              (J0min_ref=J0min_in if this is > 0)
!                          2 : RECOMMENDED: same as 1, but J0min_ref is calculated
!                              iteratively to get the right n_e at z=19.
!                              J0min_in is not relevant in this case.
! h (dble)          : H0/100
! omegab (dble)     : omega baryons
! omega0 (dble)     : omega matter total
! omegaL (dble)     : omega Lambda
! astart_sim (dble) : redshift at which you want to start the simulation
! T2_sim     (dble) : it will be in output, the T/mu in K at this redshift for contrast regions
!                     of zero density.
!
! NOTE :
! In the madau, weinberg or weinbergint cases, the J0 at high redshift is calculated as follows
!  in theuns option :
!   madau :       for z >= 15 or when the rate found is smaller than that given by
!                 theuns=.true option.
!   weinberg :    when leaving the rate table
!   weinbergint :  for z >= 8.5 or when the rate found is smaller than that given
!                 by option theuns=.true.
!   courty :
!=======================================================================
  implicit none
...
!=======================================================================
! astart : expansion factor value at start of calculation
! aend   : value of expansion factor at end of calculation
! dasura : value of da/a between 2 time steps
! h      : value of H0/100
! omegab : value of Omega baryons
! omega0 : value of Omega matiere (total)
! omegaL : Omega Lambda value
! J0min_in : value of J0min to inject :
!          If high_z_realistic_ne then it's J0min a a=astart which
!          is considered
!          Otherwise, the usual J0min is used.
!          If J0min_in <=0, the default or predefined parameters
!          are used for J0min.
! T2end  : T/mu in output
! mu     : molecular weight output
! ne     : ne in output
! if_write_result : .true. to write the evolution of temperature
!           and n_e on screen.
!=======================================================================
  implicit none
